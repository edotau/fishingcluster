...
// Pipeline config
ch_multiqc_config = file("$baseDir/resources/multiqc_config.yaml", checkIfExists: true)
...
ch_mlib_peak_count_header = file("$baseDir/resources/multiqc/mlib_peak_count_header.txt", checkIfExists: true)
ch_mlib_frip_score_header = file("$baseDir/resources/multiqc/mlib_frip_score_header.txt", checkIfExists: true)
ch_mlib_peak_annotation_header = file("$baseDir/resources/multiqc/mlib_peak_annotation_header.txt", checkIfExists: true)
ch_mlib_deseq2_pca_header = file("$baseDir/resources/multiqc/mlib_deseq2_pca_header.txt", checkIfExists: true)
ch_mlib_deseq2_clustering_header = file("$baseDir/resources/multiqc/mlib_deseq2_clustering_header.txt", checkIfExists: true)

ch_mrep_peak_count_header = file("$baseDir/resources/multiqc/mrep_peak_count_header.txt", checkIfExists: true)
ch_mrep_frip_score_header = file("$baseDir/resources/multiqc/mrep_frip_score_header.txt", checkIfExists: true)
ch_mrep_peak_annotation_header = file("$baseDir/resources/multiqc/mrep_peak_annotation_header.txt", checkIfExists: true)
ch_mrep_deseq2_pca_header = file("$baseDir/resources/multiqc/mrep_deseq2_pca_header.txt", checkIfExists: true)
ch_mrep_deseq2_clustering_header = file("$baseDir/resources/multiqc/mrep_deseq2_clustering_header.txt", checkIfExists: true)
...
if (params.gtf)       { ch_gtfls = file(params.gtf, checkIfExists: true) } else { exit 1, 'GTF annotation file not specified!' }
...
summary['Fingerprint scriptss']       = params.fingerprint_scriptss
...
process MERGED_LIB_PLOTFINGERPRINT {
    ...
    plotFingerprint \\
        ...
        --skipZeros \\
        --numberOfProcessors $task.cpus \\
        --numberOfSamples $params.fingerprint_scriptss
    """
}
...
process MERGED_LIB_CONSENSUS {
    ...
    script: // scripts are bundled with the pipeline, in nf-core/atacseq/scripts/
    suffix = 'mLb.clN'
...
workflow.onComplete {
    ...
    def tf = new File("$baseDir/resources/email_template.txt")
    def txt_template = engine.createTemplate(tf).make(email_fields)
    def email_txt = txt_template.toString()

    // Render the HTML template
    def hf = new File("$baseDir/resources/email_template.html")
    def html_template = engine.createTemplate(hf).make(email_fields)
    def email_html = html_template.toString()

    // Render the sendmail template
    def smail_fields = [ email: email_address, subject: subject, email_txt: email_txt, email_html: email_html, baseDir: "$baseDir", mqcFile: mqc_report, mqcMaxSize: params.max_multiqc_email_size.toBytes() ]
    def sf = new File("$baseDir/resources/sendmail_template.txt")
